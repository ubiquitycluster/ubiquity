...
func TestCloudReadinessRunbookDocumentsReconciliationNotObjectExistence(t *testing.T) {
	...
	for _, required := range []string{"server-side dry-run", "CRD presence", "status condition", "restore drill", "fail closed", "not object existence", "ubiquity cloud collect-readiness", "ubiquity cloud readiness", "readiness-file"} {
